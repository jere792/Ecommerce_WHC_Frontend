...
export default function AdminAjusteStock() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, tipoFiltro, fechaInicio, fechaFin]);
  ...
  const loadAjustes = async () => {
    ...
    if (data)
      setMovements(data as unknown as (Movimiento & { producto?: Producto })[]);
    setLoading(false);
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (search.length >= 3) {
      ...
      result = result.filter((m) =>
        m.producto?.nombre_producto?.toLowerCase().includes(q),
      );
    }

    if (tipoFiltro === 1) result = result.filter((m) => (m.cantidad ?? 0) > 0);
    else if (tipoFiltro === 2)
      result = result.filter((m) => (m.cantidad ?? 0) < 0);
  ...
  const columns: Column<Movimiento & { producto?: Producto }>[] = [
    ...
    },
    {
      ...
      render: (m) => (
        <span
          className={`font-semibold ${(m.cantidad ?? 0) < 0 ? "text-red-600" : "text-emerald-600"}`}
        >
          {(m.cantidad ?? 0) < 0 ? "" : "+"}
          {m.cantidad}
        </span>
    ...
    },
    {
      ...
      width: "180px",
      render: (m) => (
        <span className="text-foreground">{m.responsable || "—"}</span>
      ),
    },
    {
      header: "Observación",
      render: (m) => (
        <span className="text-foreground text-sm">{m.observacion || "—"}</span>
      ),
    },
    {
      header: "Fecha",
